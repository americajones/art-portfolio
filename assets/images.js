const images =
    [
        {
            name: "Twins",
            url: "assets/images/image6.png",
            description: "editorial piece",
            year: "2018",
            tagged: ["portfolio", "msj"]
        },
        {
            name: "Full Stack",
            url: "assets/images/image7.jpg",
            description: "poster",
            year: "2018",
            tagged: ["fanart", "portfolio"]
        },
        {
            name: "Heliophagus",
            url: "assets/images/image8.jpg",
            description: "cover",
            year: "2019",
            tagged: ["fanart", "cover", "portfolio"]
        },
        {
            name: "MSJ Winter",
            url: "assets/images/image10.jpg",
            description: "magazine cover",
            year: "2016",
            tagged: ["portfolio", "cover"]
        },
        {
            name: "Doors",
            url: "assets/images/image9.png",
            description: "editorial piece",
            year: "2018",
            tagged: ["portfolio"]
        },
        {
            name: "Lost",
            url: "assets/images/image11.jpg",
            description: "private commission",
            year: "2017",
            wideImage: true,
            tagged: ["portfolio", "commission"]
        },
        {
            name: "Offering of the Moon",
            url: "assets/images/image12.jpg",
            description: "first million hour painting",
            year: "2016",
            tagged: ["portfolio"]
        },
        {
            name: "MSJ Autumn",
            url: "assets/images/image13.jpg",
            description: "magazine cover",
            year: "2018",
            tagged: ["portfolio", "cover"]
        },
        {
            name: "Continue?",
            url: "assets/images/image14.jpg",
            description: "private commission",
            year: "2018",
            wideImage: true,
            tagged: ["portfolio", "fanart"]
        },
        {
            name: "Victorian Seduction",
            url: "assets/images/image16.jpg",
            description: "private commission",
            year: "2018",
            wideImage: true,
            tagged: ["portfolio"]
        },
        {
            name: "A Girl Called Jack",
            url: "assets/images/image18.png",
            description: "private commission",
            year: "2018",
            tagged: ["portfolio", "fanart"]
        },
        {
            name: "Thought Script",
            url: "assets/images/image17.jpg",
            description: "private commission",
            year: "2019",
            wideImage: true,
            tagged: ["portfolio"]
        },
        {
            name: "Hero",
            url: "assets/images/image3.jpg",
            description: "poster",
            year: "2019",
            tagged: ["portfolio", "fanart"]
        },
        {
            name: "Friends in Strange Places",
            url: "assets/images/image4.jpg",
            description: "anthology submission",
            year: "2019",
            tagged: ["portfolio", "greyscale"]
        },
        {
            name: "Keep Your Head Up",
            url: "assets/images/image5.jpg",
            description: "fanart",
            year: "2018",
            tagged: ["portfolio", "fanart", "haikyuu"]
        },
    ];

const sketches =
    [
        {
            name: "A Record of Past Lives",
            url: "assets/images/sketchbook/3.jpg",
            year: "2019",
            tagged: ["sketchbook MSM"]
        },
        {
            name: "twice",
            url: "assets/images/sketchbook/16.jpg", 
            year: "2020",
            tagged: ["sketchbook", "fanart"]
        },
        {
            name: "stars",
            url: "assets/images/sketchbook/image6.PNG",
            year: "2019",
            tagged: ["sketchbook", "fanart"]
        },
        {
            name: "oh no",
            url: "assets/images/sketchbook/20.png",
            year: "2019",
            tagged: ["sketchbook", "fanart"]
        },
        {
            name: "light study",
            url: "assets/images/sketchbook/21.jpg",
            year: "2018",
            tagged: ["sketchbook"]
        },
        {
            name: "long long neck",
            url: "assets/images/sketchbook/22.png",
            year: "2018",
            tagged: ["sketchbook"]
        },
        {
            name: "hm",
            url: "assets/images/sketchbook/24.png",
            year: "2018",
            tagged: ["sketchbook"]
        },
        {
            name: "you good",
            url: "assets/images/sketchbook/26.png",
            year: "2018",
            tagged: ["sketchbook"]
        },
        {
            name: "wes",
            url: "assets/images/sketchbook/28.png",
            year: "2018",
            tagged: ["sketchbook"]
        },
        {
            name: "fighto",
            url: "assets/images/sketchbook/32.jpg",
            year: "2018",
            tagged: ["sketchbook"]
        },
        {
            name: "hunger",
            url: "assets/images/sketchbook/33.jpg",
            year: "2018",
            tagged: ["sketchbook"]
        },
        {
            name: "don't believe",
            url: "assets/images/sketchbook/image15.jpg",
            year: "2019",
            tagged: ["sketchbook"]
        },
        {
            name: "dead",
            url: "assets/images/sketchbook/34.jpg",
            year: "2018",
            tagged: ["sketchbook"]
        },
        {
            name: "Off the Shelf",
            url: "assets/images/sketchbook/38.jpg",
            year: "2018",
            tagged: ["sketchbook"]
        },
        {
            name: "carrot",
            url: "assets/images/sketchbook/35.jpg",
            year: "2018",
            tagged: ["sketchbook"]
        },
        {
            name: "mother and child",
            url: "assets/images/sketchbook/36.jpg",
            year: "2019",
            tagged: ["sketchbook"]
        },
        {
            name: "trust them",
            url: "assets/images/sketchbook/image11.jpg",
            year: "2019",
            tagged: ["sketchbook"]
        },
        {
            name: "bloody nose",
            url: "assets/images/sketchbook/tamaki.jpg",
            year: "2019",
            tagged: ["sketchbook"]
        },
        {
            name: "Village of Shiminpur",
            url: "assets/images/sketchbook/39.jpg",
            year: "2019",
            tagged: ["sketchbook"]
        },

    ]

